...
class ProductModel extends Model{
    ...
    */
    public function addpro($data){       
        ...
        $data['pro_img'] = $info['savename'];//将文件名添加倒数组中
    ...
    */
    public function getList($data=1){             
        ...
        $list = D('ProductView')->where($data)->order('id DESC')->limit($Page->firstRow.','.$Page->listRows)->select(); 
         //print_r($list);exit;
        foreach($list as &$v){
            $arr = array();
           ...
           if(!empty($v['pro_img'])){
               $v['pro_img']=$this->getPath($v['img_path'])."thumb_".$arr[0];
           }
        }
        //print_r($list);exit;
    ...
    */
    ...
    public function updatePro($data){ 
        ...
        if(!empty($_FILES['pro_img']['name'][0])){            
            $info=$this->upload();
            //print_r($info);die;
            $resourse = M('Product')->find($id);
            $str = $resourse['pro_img'];
            foreach ($info as $k=>$v){
               if(($k+1)<count($info)){
    ...
    public function imgInfo($id){
        ...
        $newImg=$this->getAllpic($info['pro_img'],$imgP);//调用多张图片
